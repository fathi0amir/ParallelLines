...
# Open the image file
img = Image.open("im05.png").convert("L")  # Convert to grayscale
...
pixels_more_than_zero = sorted(pixels_more_than_zero, key=lambda x: x[1])

# Find pairs with certain values
...
for i in range(image_size - 1):
    pairs = []
    for j in range(len(pixels_more_than_zero) - 1):
        if pixels_more_than_zero[j][1] == i:
            pairs.append((pixels_more_than_zero[j]))
    if len(pairs) > 1:
        left_value = min(pairs, key=lambda x: x[0])
        right_value = max(pairs, key=lambda x: x[0])
